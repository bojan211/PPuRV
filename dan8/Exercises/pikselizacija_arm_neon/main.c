...
void perform_filtering()
{
	  int x, y, dy;
	  int8_t avg;
	  uint8x8_t pixels_row;
	  uint16x8_t row_sum;
	  uint16x4_t row_sum_16;
	  uint32x2_t row_sum_32;
	  uint64x1_t row_sum_64;
	  ...
	  for (y=0; y<YSIZE; y+= N)
	  {
	    for (x=0; x<XSIZE; x+= N)
	    {
	    	...
	    	row_sum = vdupq_n_u16(0);

	    	for( dy=0; dy<N; dy++)
	        {
	            //Calculate sum of rows, and store in int16_8_t vector
	    		pixels_row = vld1_u8((uint8_t const *) (y_pic + (y+dy+8)*(XSIZE+16)+(x+8)));
	            row_sum  = vaddw_u8(row_sum, pixels_row);
	        }
            //Do a pairwise sum of vector elements
	        row_sum_16 = vpadd_u16(vget_high_u16(row_sum), vget_low_u16(row_sum));
	        row_sum_32 = vpaddl_u16(row_sum_16);
	        row_sum_64 = vpaddl_u32(row_sum_32);
            //Average value of pixel in block
	        avg = (int8_t)(row_sum_64>>6);

	        //Fill a vector with a new pixel value
	        pixels_row = vdup_n_u8(avg);

	            // Write new big pixel value
	        for( dy=0; dy<N; dy++)
	        {
	        	vst1_u8((uint8_t *) (y_pic2 + (y+dy+8)*(XSIZE+16)+(x+8)), pixels_row);
	        }
	    }
	  }
}
